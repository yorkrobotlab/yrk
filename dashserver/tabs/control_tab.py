...
import yrk.led as led, yrk.audio as audio, yrk.display as display, yrk.motors as motors, yrk.settings as settings, yrk.utils as utils, os
...
layout = html.Div([
    ...
    create_div('Body LED',[
        html.Div([
            ...
            dcc.Dropdown(id='control-app-bodyled-dropdown',options=[{'label':i[0],'value':str(count)} for count,i in enumerate(led.solid_colours)])
        ],style=block_style),
        html.Div(id='control-app-bodyled'),
        html.Div([
            html.H6("Animation"),
            dcc.Dropdown(id='animation-dropdown',options=[{'label':i[0],'value':str(count)} for count,i in enumerate(led.body_animations)]),
        ],style=block_style),
